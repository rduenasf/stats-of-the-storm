...
function initTeamRankingPage() {
  teamRankingGeneralTemplate = Handlebars.compile(
    getTemplate("team-ranking", "#team-ranking-row-template").find("tr")[0]
      .outerHTML
  );
  teamRankingMatchTemplate = Handlebars.compile(
    getTemplate("team-ranking", "#team-ranking-match-row-template").find(
      "tr"
    )[0].outerHTML
  );
  teamRankingCCTemplate = Handlebars.compile(
    getTemplate("team-ranking", "#team-ranking-cc-row-template").find("tr")[0]
      .outerHTML
  );
  teamRankingStructureTemplate = Handlebars.compile(
    getTemplate("team-ranking", "#team-ranking-structure-row-template").find(
      "tr"
    )[0].outerHTML
  );
...
function toggleTeamRankingSection() {
  ...
  $('#team-ranking-body .section[table-name="' + section + '"]').removeClass(
    "is-hidden"
  );
...
function updateTeamRankingData(err, matches, team) {
  ...
  let mode = $("#team-ranking-body .top.attached.menu .active.item").attr(
    "data-mode"
  );
  ...
  if (mode === "averages" || mode === "total") {
    teamStats.stats.total.totalKDA =
      teamStats.takedowns.total / teamStats.deaths.total;
  } else {
  ...
  teamStats.structures.Fort.first /= Math.max(
    1,
    teamStats.structures.Fort.gamesWithFirst
  );
  teamStats.structures.Keep.first /= Math.max(
    1,
    teamStats.structures.Keep.gamesWithFirst
  );
  teamStats.structures.Fort.formatFirst = formatSeconds(
    teamStats.structures.Fort.first
  );
  teamStats.structures.Keep.formatFirst = formatSeconds(
    teamStats.structures.Keep.first
  );

  $("#team-ranking-general-table tbody").append(
    teamRankingGeneralTemplate(teamStats)
  );
  $("#team-ranking-match-table tbody").append(
    teamRankingMatchTemplate(teamStats)
  );
  $("#team-ranking-cc-table tbody").append(teamRankingCCTemplate(teamStats));
  $("#team-ranking-structure-table tbody").append(
    teamRankingStructureTemplate(teamStats)
  );
...
function layoutTeamRankingPrint(sections) {
  ...
  for (let section of sects) {
    let sectionName = $(
      '#team-ranking-body div[table-print="' + section + '"]'
    ).attr("table-name");
    addPrintPage(section);
    addPrintSubHeader(sectionName, section);
    copyFloatingTable(
      $(
        '#team-ranking-body div[table-print="' +
          section +
          '"] .floatThead-wrapper'
      ),
      getPrintPage(section)
    );
  }
}
      ...
      .modal({
        onApprove: function() {
          dialog.showSaveDialog(
            ...
            function(filename) {
              if (filename) {
                let sections = $("#team-ranking-print-sections .ui.dropdown")
                  .dropdown("get value")
                  .split(",");
