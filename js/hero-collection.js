...
function initHeroCollectionPage() {
  ...
  heroCollectionSummaryRowTemplate = Handlebars.compile(
    getTemplate(
      "hero-collection",
      "#hero-collection-hero-summary-row-template"
    ).find(".hero-collection-hero-summary-row")[0].outerHTML
  );
  heroCollectionPickRowTemplate = Handlebars.compile(
    getTemplate(
      "hero-collection",
      "#hero-collection-hero-pick-row-template"
    ).find(".hero-collection-hero-summary-row")[0].outerHTML
  );
  heroCollectionHeroWinRowTemplate = Handlebars.compile(
    getTemplate("hero-collection", "#hero-collection-detail-hero-win-row").find(
      "tr"
    )[0].outerHTML
  );
  ...
  $("#hero-collection-detail-hero-talent .talent-build table").on(
    "tablesort:complete",
  ...
  $("#hero-collection-summary table th.stat").data("sortBy", function(
    th,
    td,
    tablesort
  ) {
  ...
  $("#hero-collection-picks table th.stat").data("sortBy", function(
    th,
    td,
    tablesort
  ) {
  ...
  for (let m in heroCollectionHeroDataFilter.mode.$in) {
    filterWidget
      ...
      .dropdown("set selected", heroCollectionHeroDataFilter.mode.$in[m]);
  }
  ...
  $("#hero-collection-body .five.ui.buttons .button").click(function() {
    toggleHeroCollectionType(
      "#hero-collection-body",
      $(this),
      "#hero-collection-body"
    );
  });
  ...
  $(
    "#hero-collection-detail-with-summary .cache-collections.dropdown"
  ).dropdown({
    fullTextSearch: true,
  ...
  $(
    "#hero-collection-detail-against-summary .cache-collections.dropdown"
  ).dropdown({
    fullTextSearch: true,
    onChange: function(value, text, $elem) {
      updateHeroCollectionVsStats(
...
function updateCollectionFilter(map, hero) {
  ...
  loadOverallHeroCollectionData();
  loadHeroCollectionData(
    $("#hero-collection-hero-select-menu").dropdown("get value"),
    null,
    null
  );
}
...
function resetCollectionFilter() {
  ...
  let filterWidget = $(
    '.filter-popup-widget[widget-name="hero-collection-filter"]'
  );
  for (let m in heroCollectionHeroDataFilter.mode.$in) {
    filterWidget
      .find(".filter-widget-mode")
...
function loadOverallHeroCollectionData() {
  heroCollectionHeroMatchThresh = parseInt(
    $("#hero-collection-hero-thresh input").val()
  );
  ...
  DB.getMatches(heroCollectionMapDataFilter, function(err, docs) {
    ...
    for (let h in overallStats) {
      ...
      $("#hero-collection-summary tbody").append(
        heroCollectionSummaryRowTemplate(context)
      );
      ...
      banContext.format.firstBanPercent = formatStat(
        "pct",
        banContext.firstBanPercent
      );
      banContext.secondBanPercent =
        hero.bans.second / overallStats.totalMatches;
      banContext.format.secondBanPercent = formatStat(
        "pct",
        banContext.secondBanPercent
      );
      ...
      for (let p in banContext.picks) {
        banContext.picks[p].pct =
          banContext.picks[p].count / overallStats.totalMatches;
        banContext.picks[p].formatPct = formatStat(
          "pct",
          banContext.picks[p].pct
        );
      }

      $("#hero-collection-picks tbody").append(
        heroCollectionPickRowTemplate(banContext)
      );
    }
    ...
    for (let r of Heroes.roles) {
      ...
      if (!(r in roleData)) {
        ...
        selector
          .find('div[name="pool"] .value')
          .text(roleData[r].count + " / " + Heroes.heroRoleCount(r));
        selector.find('div[name="games"] .value').text(roleData[r].games);
        selector
          .find('div[name="ban"] .value')
          .text(roleData[r].bans + " / " + Heroes.heroRoleCount(r));
      }
    }

    $('#hero-collection-pool div[role="all"] div[name="pool"] .value').text(
      totalCount + " / " + Heroes.heroCount
    );
    $('#hero-collection-pool div[role="all"] div[name="ban"] .value').text(
      totalBan + " / " + Heroes.heroCount
    );
    ...
    $("#hero-collection-page-content th").removeClass(
...
function loadHeroCollectionData(value, text, $elem) {
  ...
  DB.getHeroData(query, function(err, docs) {
    ...
    renderHeroTalentsTo(
      query.hero,
      $("#hero-collection-detail-hero-talent"),
      docs
    );
    ...
    // vs stats
    let val = $(
      "#hero-collection-detail-with-summary .cache-collections"
    ).dropdown("get value");
    updateHeroCollectionVsStats(
      val,
      $(
        '#hero-collection-detail-with-summary .cache-collections .item[data-value="' +
          val +
          '"]'
      ),
      "with",
    ...
    val = $(
      "#hero-collection-detail-against-summary .cache-collections"
    ).dropdown("get value");
    updateHeroCollectionVsStats(
      val,
      $(
        '#hero-collection-detail-against-summary .cache-collections .item[data-value="' +
          val +
          '"]'
      ),
      "against",
    ...
    // these are annoying
    $(
      '#hero-collection-detail-misc-summary .statistic[name="overallWin"] .value'
    ).text(formatStat("pct", stats.wins / stats.games));
    $(
      '#hero-collection-detail-misc-summary .statistic[name="overallGames"] .value'
    ).text(stats.games);
    $(
      '#hero-collection-detail-misc-summary .statistic[name="overallTD"] .value'
    ).text(formatStat("overallTD", stats.totalTD / stats.games), true);
    $(
      '#hero-collection-detail-misc-summary .statistic[name="overallDeaths"] .value'
    ).text(formatStat("overallDeaths", stats.totalDeaths / stats.games), true);
    $(
      '#hero-collection-detail-misc-summary .statistic[name="overallKDA"] .value'
    ).text(formatStat("KDA", stats.totalTD / Math.max(stats.totalDeaths, 1)));
    $(
      '#hero-collection-detail-misc-summary .statistic[name="overallMVP"] .value'
    ).text(formatStat("pct", stats.totalMVP / Math.max(stats.games, 1)));
    $(
      '#hero-collection-detail-misc-summary .statistic[name="overallAward"] .value'
    ).text(formatStat("pct", stats.totalAward / Math.max(stats.games)));
...
function updateHeroCollectionVsStats(value, $elem, key, container) {
  ...
  if ($elem.attr("data-type") === "external")
    DB.getExternalCacheCollectionHeroStats(cid, function(cache) {
      renderHeroCollectionVsStatsTo(
        container,
        heroDataWinCache[key],
        heroCollectionHeroMatchThresh,
        cache
      );
      container
        .find(".dropdown.cache-collections")
...
function renderHeroCollectionVsStatsTo(container, stats, threshold, avg) {
  ...
  for (let h in stats) {
    ...
    if (h in avg.heroData.heroes) {
      context.avgDelta =
        context.winPercent -
        avg.heroData.heroes[h].wins / avg.heroData.heroes[h].games;
    } else {
      context.avgDelta = 0;
    }

    context.formatAvgDelta =
      (context.avgDelta > 0 ? "+" : "") + formatStat("pct", context.avgDelta);
    context.deltaClass =
      context.avgDelta > 0 ? "plus" : context.avgDelta < 0 ? "minus" : "";
...
function layoutHeroCollectionPrint(sections) {
  ...
  if (sects.indexOf("general") !== -1) {
    ...
    copyFloatingTable(
      $("#hero-collection-summary .floatThead-wrapper"),
      getPrintPage("general")
    );
  }
  ...
  if (sects.indexOf("draft") !== -1) {
    ...
    copyFloatingTable(
      $("#hero-collection-picks .floatThead-wrapper"),
      getPrintPage("draft")
    );
  }
  ...
  if (sects.indexOf("comps") !== -1) {
    ...
    copyFloatingTable(
      $("#hero-collection-comps .floatThead-wrapper"),
      getPrintPage("comps")
    );
  }
  ...
  if (sects.indexOf("pool") !== -1) {
    ...
    getPrintPage("pool")
      .find(".hero-pool.segment")
      .append($("#hero-collection-pool .six.column.grid").clone());

    getPrintPage("pool").append(
      '<div class="ui segment draft-tables"><div class="ui three column grid"></div></div>'
    );
    ...
    copyFloatingTable(
      $("#hero-collection-zero-participation .floatThead-wrapper"),
      $("#print-window .grid-cell-1")
    );
    copyFloatingTable(
      $("#hero-collection-zero-games .floatThead-wrapper"),
      $("#print-window .grid-cell-2")
    );
    copyFloatingTable(
      $("#hero-collection-zero-bans .floatThead-wrapper"),
      $("#print-window .grid-cell-3")
    );
  }
...
function layoutHeroCollectionDetailPrint() {
  ...
  $("#print-window .contents").append(
    $("#hero-collection-hero-header h2.header").clone()
  );

  addPrintSubHeader("Overall Stats");
  $("#print-window .contents").append(
    $("#hero-collection-detail-misc-summary .statistics").clone()
  );
  $("#print-window")
    .find(".statistics")
  ...
  copyFloatingTable(
    $("#hero-collection-detail-hero-talent .talent-pick .floatThead-wrapper"),
  ...
  copyFloatingTable(
    $("#hero-collection-detail-hero-talent .talent-build .floatThead-wrapper"),
    getPrintPage("builds")
  );
  ...
  copyFloatingTable(
    $("#hero-collection-detail-map-summary .floatThead-wrapper"),
    getPrintPage("maps")
  );
  ...
  copyFloatingTable(
    $("#hero-collection-detail-with-summary .floatThead-wrapper"),
    getPrintPage("with")
  );
  ...
  copyFloatingTable(
    $("#hero-collection-detail-against-summary .floatThead-wrapper"),
  ...
  copyFloatingTable(
    $("#hero-collection-award-summary .floatThead-wrapper"),
    getPrintPage("awards")
  );
      ...
      .modal({
        onApprove: function() {
          dialog.showSaveDialog(
            ...
            function(filename) {
              if (filename) {
                let sections = $("#hero-collection-print-sections .ui.dropdown")
                  .dropdown("get value")
                  .split(",");
