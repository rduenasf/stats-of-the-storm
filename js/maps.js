...
function initMapsPage() {
  // templates
  mapsMapRowTemplate = Handlebars.compile(
    getTemplate("maps", "#map-table-row-template").find("tr")[0].outerHTML
  );
  ...
  $("#maps-page-content table th.stat").data("sortBy", function(
    th,
    td,
    tablesort
  ) {
...
function loadMapStats() {
  ...
  DB.getMatches(mapsMapDataFilter, function(err, docs) {
    ...
    updateTeamStat(
      statContainer,
      "average",
      formatSeconds(mapData.aggregate.average)
    );
    updateTeamStat(
      statContainer,
      "median",
      formatSeconds(mapData.aggregate.median)
    );
    ...
    updateTeamStat(
      ...
      formatStat(
        "pct",
        mapData.aggregate.firstPickWin / mapData.aggregate.draftGames
      )
    );
    updateTeamStat(
      statContainer,
      ...
      formatStat(
        "pct",
        mapData.aggregate.firstObjectiveWins / mapData.aggregate.games
      )
    );
    updateTeamStat(
      statContainer,
      "blueWin",
      formatStat("pct", mapData.aggregate.blueWin / mapData.aggregate.games)
    );
    updateTeamStat(
      statContainer,
      "redWin",
      formatStat("pct", mapData.aggregate.redWin / mapData.aggregate.games)
    );

    $('#map-overall-stats .statistic[name="min"]').attr(
      "matchID",
      mapData.aggregate.minId
    );
    $('#map-overall-stats .statistic[name="max"]').attr(
      "matchID",
      mapData.aggregate.maxId
    );
    ...
    for (let map in mapData.stats) {
      ...
      if (
        map === ReplayTypes.MapType.BlackheartsBay ||
        map === ReplayTypes.MapType.HauntedMines
      ) {
        context.firstObjectiveWinsPct = NaN;
      } else {
        context.firstObjectiveWinsPct =
          context.firstObjectiveWins / context.games;
      }
