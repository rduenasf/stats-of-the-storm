...
function initMatchDetailPage() {
  ...
  // templates
  matchSummaryRowTemplate = Handlebars.compile(
    getTemplate("match-detail", "#match-detail-summary-row-template").find(
      "tr"
    )[0].outerHTML
  );
  matchDetailHeaderTemplate = Handlebars.compile(
    getTemplate("match-detail", "#match-detail-detail-header").find("th")[0]
      .outerHTML
  );
  matchDetailRowTemplate = Handlebars.compile(matchDetailRowTemplateSrc);
  matchTalentRowTitleTemplate = Handlebars.compile(
    getTemplate(
      "match-detail",
      "#match-detail-talents-row-title-template"
    ).find("tr")[0].outerHTML
  );
  matchTalentRowCellTemplate = Handlebars.compile(
    getTemplate("match-detail", "#match-detail-talents-row-cell-template").find(
      "td"
    )[0].outerHTML
  );
  matchChatEntryTemplate = Handlebars.compile(
    getTemplate("match-detail", "#match-detail-chat-log-entry").find(
      "div.event"
    )[0].outerHTML
  );
  matchTauntEntryTemplate = Handlebars.compile(
    getTemplate("match-detail", "#match-detail-taunt-entry").find("tr")[0]
      .outerHTML
  );
  ...
  $("#player-hero-detail-stats table th.time-sort").data("sortBy", function(
    th,
    td,
    tablesort
  ) {
  ...
  overallLevelGraph = new Chart(
    $("#match-detail-levels-xp"),
    overallLevelGraphData
  );
  ...
  for (let g of matchDetailTimelineGroups.getIds()) {
    if (matchDetailTimelineGroups.get(g).visible) {
      $(
        '#match-detail-timeline-buttons .button[button-id="' + (g - 1) + '"]'
      ).addClass("violet");
    }
  }
...
function matchDetailTeamAction(action) {
  if (action === "new-from-blue" || action === "new-from-red") {
    ...
    $("#match-detail-existing-team")
      .modal({
        onApprove: function() {
          let team = $("#match-detail-existing-team .team-menu").dropdown(
            "get value"
          );
  ...
  for (let h of matchDetailMatch.heroes) {
    if (Heroes.role(h) === "") {
      showMessage(
        ...
        "A hero name in this match's metadata is invalid. This only affects the ability to search for a hero in this match on the Matches page. Player Details and the Collection: Heroes pages are unaffected. To fix, you should re-import the match or re-create the database.",
...
function updateBasicInfo() {
  ...
  popuplateMatchDetailTeamNameplate(
    matchDetailMatch._id,
    0,
    matchDetailMatch.teams[0].ids
  );
  popuplateMatchDetailTeamNameplate(
    matchDetailMatch._id,
    1,
    matchDetailMatch.teams[1].ids
  );
  ...
  if (matchDetailMatch.winner === 0) {
    winnerHeader
      .removeClass("red")
      .addClass("blue")
      .text("Blue Team Victory");
  } else {
    winnerHeader
      .removeClass("blue")
      .addClass("red")
      .text("Red Team Victory");
  }

  $("#match-detail-mode").text(
    ReplayTypes.GameModeStrings[matchDetailMatch.mode]
  );
  ...
  if (matchDetailMatch.mode !== ReplayTypes.GameMode.QuickMatch) {
    ...
    for (let t in matchDetailMatch.bans) {
      ...
      for (let b in bans) {
        ...
        $('div[ban-slot="' + slot + '"] img').attr(
          "src",
          "assets/heroes-talents/images/heroes/" + icon
        );
      }
    }
    ...
    if (
      ...
      for (let t in [0, 1]) {
        ...
        for (let p = 0; p < picks.length; p++) {
          let h = picks[p];
          let slot =
            (parseInt(t) === first ? "first" : "second") + "-" + (p + 1);
          let icon = Heroes.heroIcon(h);
          $('div[pick-slot="' + slot + '"] img').attr(
            "src",
            "assets/heroes-talents/images/heroes/" + icon
          );
        }
    ...
    $('div[pick-slot^="first"]')
      .removeClass(secondClass)
      .addClass(firstClass);
    $('div[pick-slot^="first"] .label')
      .removeClass(secondClass)
      .addClass(firstClass);
    $('div[pick-slot^="second"]')
      .removeClass(firstClass)
      .addClass(secondClass);
    $('div[pick-slot^="second"] .label')
      .removeClass(firstClass)
      .addClass(secondClass);
    $('div[ban-slot^="first"]')
      .removeClass(secondClass)
      .addClass(firstClass);
    $('div[ban-slot^="first"] .label')
      .removeClass(secondClass)
      .addClass(firstClass);
    $('div[ban-slot^="second"]')
      .removeClass(firstClass)
      .addClass(secondClass);
    $('div[ban-slot^="second"] .label')
      .removeClass(firstClass)
      .addClass(secondClass);
  } else {
  ...
  DB.getTeamByPlayers(players, function(err, docs) {
    if (docs.length > 0) {
      ...
      $("#match-detail-summary")
        .find(elem)
        .text(team.name);
      $("#match-detail-summary .match-detail-team-names").removeClass(
        "is-hidden"
      );
    } else {
...
function appendSummaryRow(color, id) {
  ...
  $("#match-detail-summary table tbody").append(
    matchSummaryRowTemplate(context)
  );
...
function appendDetailHeader(color, id) {
  ...
  $("#match-detail-details table thead tr").append(
    matchDetailHeaderTemplate(context)
  );
}
...
function appendDetailRow(field) {
  ...
  $("#match-detail-details table tbody").append(
    matchDetailRowTemplate(context)
  );
}
...
function graphXP() {
  ...
  let maxXp = Math.max(
    team0XP[team0XP.length - 1].y,
    team1XP[team1XP.length - 1].y
  );
...
function getLevelsXPSet(teamID) {
  ...
  data.push({
    x: matchDetailMatch.length,
    y: matchDetailMatch.teams[teamID].level
  });
...
function loadTimeline() {
  ...
  for (let i = 0; i < adv.length; i++) {
    ...
    if (newLevelDiff !== currentLevelDiff) {
      ...
      item.className =
        item.className + " level-adv plus" + Math.abs(currentLevelDiff);
  ...
  lastItem.className =
    lastItem.className + " level-adv plus" + Math.abs(lastLevelDiff);
  ...
  if (matchDetailMatch.map === ReplayTypes.MapType.ControlPoints) {
    ...
    matchDetailMatch.map === ReplayTypes.MapType.BattlefieldOfEternity
  ) {
    getBOEEvents(items);
  }
  ...
  opts.format = {
    minorLabels: function(date, scale, step) {
      ...
      return formatSeconds(
        date._d.getUTCMilliseconds() + date._d.getUTCSeconds() * 1000
      );
    }
  };
...
function toggleGroup(idx) {
  let elem = $(
    '#match-detail-timeline-buttons .button[button-id="' + idx + '"]'
  );
...
function generateTDTimeline(data) {
  ...
  pop +=
    "<div class='content'>" +
    data.victim.hero +
    "<div class='sub header player-name'>Killed at ";
  ...
  for (let a in data.killers) {
    ...
    pop +=
      "<img class='ui image' src='assets/heroes-talents/images/heroes/" +
      Heroes.heroIcon(k.hero) +
      "'>";
  }
  pop += "</div>";
...
function getBraxisEvents(items) {
  for (let w in matchDetailMatch.objective.waves) {
    ...
    if ("startTime" in wave) {
      ...
      item0.content =
        '<div class="timeline-popup" data-variation="wide" data-html="' +
        pop0 +
        '">' +
        t0 +
        "</div>";
...
function getGardenEvents(items) {
  for (let t in matchDetailMatch.objective) {
    for (let i in matchDetailMatch.objective[t].events) {
      ...
      let hero =
        matchDetailMatch.teams[t].heroes[
          matchDetailMatch.teams[t].ids.indexOf(terror.player)
        ];
...
function getTowersEvents(items) {
  for (let t in ["0", "1"]) {
    for (let i in matchDetailMatch.objective[t].events) {
      ...
      item.content =
        '<div class="timeline-popup" data-html="' +
        pop +
        '">Altar Capture</div>';
      items.push(item);
    }
...
function getMinesEvents(items) {
  for (let t in [0, 1]) {
    for (let g in matchDetailMatch.objective[t]) {
      ...
      pop +=
        ", Duration: " + formatSeconds(golem.duration) + "</div></div></h3>";
...
function getBOEEvents(items) {
  for (let i in matchDetailMatch.objective.results) {
    ...
    let pop =
      "<h3 class='ui header'><div class='content'>" +
      formatStat("", immo.power, true) +
      "% Immortal";
    pop +=
...
function getDragonEvents(items) {
  for (let t in [0, 1]) {
    for (let d in matchDetailMatch.objective[t].events) {
      ...
      let hero =
        matchDetailMatch.teams[t].heroes[
          matchDetailMatch.teams[t].ids.indexOf(dragon.player)
        ];
      let pop = "<h3 class='ui image header'>";
      pop +=
        "<img src='assets/heroes-talents/images/heroes/" +
...
function getShrinesEvents(items) {
  ...
  for (let t in [0, 1]) {
    for (let p in matchDetailMatch.objective[t].events) {
      ...
      let pop =
        "<h3 class='ui header'><div class='content'>" +
        punisher.type.substr(0, punisher.type.length - 6) +
        " Punisher";
      pop +=
        "<div class='sub header'>Spawn: " +
        formatSeconds(item.start) +
      ...
      item.content =
        ...
        '">';
      item.content +=
        punisher.type.substr(0, punisher.type.length - 6) + " Punisher</div>";
      items.push(item);
    }
  }
}
...
function getWarheadEvents(items) {
  for (let t in [0, 1]) {
    for (let w in matchDetailMatch.objective[t].events) {
      ...
      let hero =
        matchDetailMatch.teams[t].heroes[
          matchDetailMatch.teams[t].ids.indexOf(nuke.player)
        ];
      let pop = "<h3 class='ui image header'>";
      pop +=
        "<img src='assets/heroes-talents/images/heroes/" +
        ...
        "' class='ui large circular image'>";
      pop +=
        "<div class='content'>Nuke" +
        (nuke.success ? "" : " Attempt") +
...
function initTeamStatGraphs() {
  ...
  teamOverallStatGraph = new Chart(
    $("#match-detail-team-numbers"),
    teamOverallStatGraphData
  );
  teamfightStatGraph = new Chart(
    $("#match-detail-teamfight-numbers"),
    teamfightStatGraphData
  );
  teamCCGraph = new Chart($("#match-detail-team-cc"), teamCCGraphData);
  timePerTierGraph = new Chart(
    $("#match-detail-time-per-tier"),
    timePerTierGraphData
  );
}
...
function drawTeamStatGraphs() {
  ...
  for (let i in intervals) {
    ...
    if (interval[1] in team0Levels) {
      timePerTierGraphData.data.datasets[0].data.push(
        team0Levels[interval[1]].time - team0Levels[interval[0]].time
      );
    } else if (interval[0] in team0Levels && !(interval[1] in team0Levels)) {
      // end of game
      timePerTierGraph.data.datasets[0].data.push(
        matchDetailMatch.length - team0Levels[interval[0]].time
      );
    } else {
    ...
    if (interval[1] in team1Levels) {
      timePerTierGraphData.data.datasets[1].data.push(
        team1Levels[interval[1]].time - team1Levels[interval[0]].time
      );
    } else if (interval[0] in team1Levels && !(interval[1] in team1Levels)) {
      // end of game
      timePerTierGraph.data.datasets[1].data.push(
        matchDetailMatch.length - team1Levels[interval[0]].time
      );
    } else {
...
function updateTeamStats() {
  for (let t in [0, 1]) {
    ...
    let elem =
      t === "0"
        ? $("#match-detail-blue-team-stats")
        : $("#match-detail-red-team-stats");
    ...
    updateTeamStat(
      elem,
      "merc-uptime-pct",
      formatStat("mercUptimePercent", stats.mercUptimePercent)
    );

    updateTeamStat(
      elem,
      "forts-destroyed",
      stats.structures.Fort.destroyed ? stats.structures.Fort.destroyed : "0"
    );
    updateTeamStat(elem, "forts-lost", stats.structures.Fort.lost);
    updateTeamStat(
      elem,
      "first-fort",
      stats.structures.Fort.destroyed === 0
        ? "N/A"
        : formatSeconds(stats.structures.Fort.first)
    );
    ...
    if (matchDetailMatch.map === ReplayTypes.MapType.TowersOfDoom) {
      updateTeamStat(
        elem,
        "wells-destroyed",
        stats.structures["Fort Well"].destroyed
      );
      ...
      updateTeamStat(
        ...
        hideWellTime
          ? "N/A"
          : formatSeconds(stats.structures["Fort Well"].first)
      );

      updateTeamStat(
        elem,
        "towers-destroyed",
        stats.structures["Fort Tower"].destroyed
      );
      ...
      updateTeamStat(
        ...
        hideTowerTime
          ? "N/A"
          : formatSeconds(stats.structures["Fort Tower"].first)
      );
      ...
      updateTeamStat(
        elem,
        "keeps-destroyed",
        stats.structures.Keep.destroyed ? stats.structures.Keep.destroyed : "0"
      );
      updateTeamStat(elem, "keeps-lost", stats.structures.Keep.lost);
      updateTeamStat(
        elem,
        "first-keep",
        stats.structures.Keep.destroyed === 0
          ? "N/A"
          : formatSeconds(stats.structures.Keep.first)
      );
      ...
      updateTeamStat(
        ...
        "wells-destroyed",
        stats.structures["Fort Well"].destroyed +
          stats.structures["Keep Well"].destroyed
      );
      updateTeamStat(
        elem,
        "wells-lost",
        stats.structures["Fort Well"].lost + stats.structures["Keep Well"].lost
      );
      ...
      updateTeamStat(
        ...
        hideWellTime
          ...
          : formatSeconds(
              Math.min(
                stats.structures["Fort Well"].first,
                stats.structures["Keep Well"].first
              )
            )
      );
      ...
      updateTeamStat(
        ...
        "towers-destroyed",
        stats.structures["Fort Tower"].destroyed +
          stats.structures["Keep Tower"].destroyed
      );
      updateTeamStat(
        elem,
        "towers-lost",
        stats.structures["Fort Tower"].lost +
          stats.structures["Keep Tower"].lost
      );
      ...
      updateTeamStat(
        ...
        hideTowerTime
          ...
          : formatSeconds(
              Math.min(
                stats.structures["Fort Tower"].first,
                stats.structures["Keep Tower"].first
              )
            )
      );
    }
...
function matchDetailCollectionAction(value, text, $elem) {
  if (value === "add") {
    $("#matches-collection-select .header").text(
      "Add Current Match To Collection"
    );
    ...
    $("#matches-collection-select .header").text(
      "Remove Current Match From Collection"
    );
    $("#matches-collection-select p.text").text(
      "Remove the current match from the spcified collection."
    );
      ...
      .modal({
        onApprove: function() {
          dialog.showSaveDialog(
            ...
            function(filename) {
              if (filename) {
                let sections = $("#match-detail-print-sections .ui.dropdown")
                  .dropdown("get value")
                  .split(",");
...
function layoutMatchDetailPrint(sections) {
  ...
  if (sects.indexOf("summary") !== -1) {
    ...
    if (!$("#match-detail-draft").hasClass("is-hidden")) {
      ...
      getPrintPage("summary")
        .find(".top.attached.label")
        .remove();
    }
  }
  ...
  if (sects.indexOf("team-stats") !== -1) {
    ...
    getPrintPage("team-stats")
      .find(".grid")
      .append($(".print-team-stats").clone());
  }
  ...
  if (sects.indexOf("graphs") !== -1) {
    ...
    copyGraph(
      teamOverallStatGraphData,
      $("#print-window #match-detail-team-numbers"),
  ...
  if (sects.indexOf("details") !== -1) {
    ...
    copyFloatingTable(
      $('.ui.tab[data-tab="details"] .floatThead-wrapper'),
      getPrintPage("details")
    );
  }
  ...
  if (sects.indexOf("xp") !== -1) {
    ...
    $("#print-window .contents .xp.page").append(
      '<div class="ui two column grid xp-graphs"></div>'
    );

    $("#print-window .xp-graphs").append(
      '<div class="column xp-vs-time"><h3 class="ui dividing header">XP vs Time</h3><canvas></div>'
    );
    copyGraph(overallXPGraphData, $("#print-window .xp-vs-time canvas"), {
      width: 700,
      height: 400
    });
    ...
    copyGraph(
      overallLevelGraphData,
      $("#print-window .levels-vs-time canvas"),
    ...
    copyGraph(blueTeamXPGraphData, $("#print-window .blue-xpb canvas"), {
      width: 700,
      height: 400
    });
    ...
    copyGraph(redTeamXPGraphData, $("#print-window .red-xpb canvas"), {
      width: 700,
      height: 400
    });

    addPrintPage("xp2");
    $("#print-window .contents .xp2.page").append(
      '<div class="ui two column grid xp-graphs2"></div>'
    );
    $("#print-window .xp-graphs2").append(
      '<div class="column soak-xp"><h3 class="ui dividing header">XP Soak</h3><canvas></div>'
    );
    copyGraph(teamXPSoakGraphData, $("#print-window .soak-xp canvas"), {
      width: 700,
      height: 400
    });
    ...
    copyGraph(timePerTierGraphData, $("#print-window .xp-time canvas"), {
      width: 700,
      height: 400
    });
  }
  ...
  if (sects.indexOf("taunts") !== -1) {
    ...
    copyFloatingTable(
      $("#match-detail-chat-log .floatThead-wrapper"),
      getPrintPage("taunt")
    );
  }
