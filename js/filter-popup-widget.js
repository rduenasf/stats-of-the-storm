...
function initPopup(elem) {
  ...
  elem
    .find(".filter-widget-start-date")
    .datepicker("setDate", new Date("1-1-2012"));
  ...
  elem.find(".filter-widget-season").dropdown({
    onChange: function(value, text, $item) {
      if (value !== "0" && value !== "") {
        elem
          .find(".filter-widget-start-date")
          .datepicker("setDate", ReplayTypes.SeasonDates[text].start);
        elem
          .find(".filter-widget-end-date")
          .datepicker("setDate", ReplayTypes.SeasonDates[text].end);
      } else {
        elem
          .find(".filter-widget-start-date")
          .datepicker("setDate", new Date("1-1-2012"));
  ...
  for (let s in ReplayTypes.SeasonDates) {
    elem
      .find(".filter-widget-season .menu")
      .prepend('<div class="item">' + s + "</div>");
  }
  elem
    .find(".filter-widget-season .menu")
    .prepend('<div class="item" data-value="0">None</div>');
...
function getPopupQuery(elem, callback) {
  ...
  let heroes = elem
    .find(".filter-widget-hero-search")
    .dropdown("get value")
    .split(",");

  // maps
  let maps = elem
    .find(".filter-widget-map")
    .dropdown("get value")
    .split(",");

  // patches
  let patches = elem
    .find(".filter-widget-patch")
    .dropdown("get value")
    .split(",");
  ...
  // tags
  let tags = elem
    .find(".filter-widget-tags")
    .dropdown("get value")
    .split(",");

  // toggles;
  let widgetHeroOnTeam = elem
    .find(".filter-widget-hero-team")
    .checkbox("is checked");
