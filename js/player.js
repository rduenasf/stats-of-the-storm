...
function initPlayerPage() {
  ...
  // templates
  playerDetailHeroSummaryRowTemplate = Handlebars.compile(
    getTemplate("player", "#player-detail-hero-summary-row").find("tr")[0]
      .outerHTML
  );
  playerDetailMapSummaryRowTemplate = Handlebars.compile(
    getTemplate("player", "#player-detail-map-summary-row").find("tr")[0]
      .outerHTML
  );
  playerWinRateRowTemplate = Handlebars.compile(
    getTemplate("player", "#player-detail-player-win-row").find("tr")[0]
      .outerHTML
  );
  heroWinRateRowTemplate = Handlebars.compile(
    getTemplate("player", "#player-detail-hero-win-row").find("tr")[0].outerHTML
  );
  heroTalentRowTemplate = Handlebars.compile(
    getTemplate("player", "#player-detail-talent-row").find("tr")[0].outerHTML
  );
  playerAwardRowTemplate = Handlebars.compile(
    getTemplate("player", "#player-detail-hero-award-row").find("tr")[0]
      .outerHTML
  );
  playerCompareRowTemplate = Handlebars.compile(
    getTemplate("player", "#player-compare-table-row").find("tr")[0].outerHTML
  );
  playerHeroDetailRowTemplate = Handlebars.compile(
    playerHeroDetailRowTemplateContent
  );
  ...
  $("#player-detail-hero-summary table th.stat").data("sortBy", function(
    th,
    td,
    tablesort
  ) {
  ...
  $("#player-hero-detail-stats table th.stat").data("sortBy", function(
    th,
    td,
    tablesort
  ) {
  ...
  $("#player-hero-detail-stats table").on("tablesort:complete", function(
    event,
    tablesort
  ) {
  ...
  $("#player-detail-hero-talent .talent-build table").on(
    "tablesort:complete",
    function(event, tablesort) {
  ...
  progressionWinRateGraph = new Chart(
    $("#player-progression-win-rate"),
    progressionWinRateGraphData
  );
  progressionStatGraph = new Chart(
    $("#player-progression-kda"),
    progressionStatGraphData
  );
  progressionAwardsGraph = new Chart(
    $("#player-progression-awards"),
    progressionAwardsGraphData
  );
  heroPoolGamesGraph = new Chart(
    $("#player-hero-pool-games-graph"),
    heroPoolGamesGraphData
  );
  heroPoolWinsGraph = new Chart(
    $("#player-hero-pool-win-graph"),
    heroPoolWinsGraphData
  );
...
function initPlayerStatGraphMenus() {
  ...
  $("#player-progression .player-axis-settings .green.button").click(
    renderProgression
  );
}
...
function createDetailTableHeader() {
  ...
  $("#player-hero-detail-stats thead tr").append(
    '<th style="width: 500px;">Hero</th>'
  );
  $("#player-hero-detail-stats thead tr").append("<th>Games</th>");
  for (let i in allDetailStats) {
    $("#player-hero-detail-stats thead tr").append(
      '<th class="stat">' + DetailStatString[allDetailStats[i]] + "</th>"
    );
  }
}
...
function updatePlayerPage(err, doc) {
  if (doc.length === 1) {
    ...
    DB.getPlayerTeams(playerDetailInfo._id, function(err, teams) {
      ...
      $("#player-page-header h1.header .content .teams").text(
        teamNames.join(", ")
      );
    });
    ...
    DB.getHeroDataForPlayerWithFilter(
      playerDetailInfo._id,
      playerDetailFilter,
      processPlayerData
    );
  } else {
...
function processPlayerData(err, docs) {
  ...
  $("#player-hero-select-menu .menu").html(
  ...
  updatePlayerCollectionCompare(
    val,
    null,
    $('#player-compare-collection .menu .item[data-value="' + val + '"]')
  );
...
function renderAllHeroSummary() {
  ...
  for (let h in playerDetailStats.heroes) {
    ...
    context.winPercent =
      playerDetailStats.heroes[h].wins / playerDetailStats.heroes[h].games;
    ...
    if (context.games >= playerHeroMatchThreshold)
      $("#player-detail-hero-summary tbody").append(
        playerDetailHeroSummaryRowTemplate(context)
      );
  ...
  for (let r of Heroes.roles) {
    ...
    if (!(r in roleData)) {
      ...
      selector
        ...
        .text("0 / " + Heroes.heroRoleCount(r));

      heroPoolGamesGraphData.data.datasets[0].data.push(0);
      heroPoolWinsGraphData.data.datasets[0].data.push(0);
    } else {
      selector
        .find('div[name="games"] .value')
        .text(formatStat("", roleData[r].games, true));
      selector
        .find('div[name="win"] .value')
        .text(formatStat("pct", roleData[r].wins / roleData[r].games));
      selector
        .find('div[name="pool"] .value')
        .text(
          formatStat("", roleData[r].count, true) +
            " / " +
            Heroes.heroRoleCount(r)
        );
  ...
  $("#player-overall-hero-pool .value").text(
    Object.keys(playerDetailStats.heroes).length + " / " + Heroes.heroCount
  );
}
...
function renderHeroTalentsTo(hero, container, docs) {
  ...
  for (let tier in data) {
    container
      ...
      .append(
        '<tr class="level-header"><td colspan="4">Level ' +
          tierToLevel[tier] +
          "</td></tr>"
      );
    let total = 0;
    ...
    for (let talent in data[tier]) {
      ...
      context.formatWinPercent = formatStat(
        "pct",
        data[tier][talent].wins / context.games
      );

      container
        .find(".talent-pick tbody")
        .append(heroTalentRowTemplate(context));
    }
  }
  ...
  for (let b in builds) {
    ...
    row.append(
      '<td data-sort-value="' +
        winPct +
        '">' +
        formatStat("pct", winPct) +
        "</td>"
    );
    row.append(
      '<td data-sort-value="' + build.games + '">' + build.games + "</td>"
    );
    row.append(
      '<td data-sort-value="' + pop + '">' + formatStat("pct", pop) + "</td>"
    );
...
function renderPlayerSummary() {
  ...
  for (let d in playerDetailStats.withPlayer) {
    ...
    $("#player-detail-friend-summary tbody").append(
      playerWinRateRowTemplate(context)
    );
  }

  for (let d in playerDetailStats.againstPlayer) {
    if (
      playerDetailStats.againstPlayer[d].games < settings.get("playerThreshold")
    )
      continue;
    ...
    $("#player-detail-rival-summary tbody").append(
      playerWinRateRowTemplate(context)
    );
  }

  renderHeroVsStatsTo(
    $("#player-detail-with-summary"),
    playerDetailStats.withHero,
    playerHeroMatchThreshold
  );
  renderHeroVsStatsTo(
    $("#player-detail-against-summary"),
  ...
  for (let s in playerDetailStats.skins) {
    ...
    $("#player-detail-skin-summary tbody").append(
      playerWinRateRowTemplate(context)
    );
  }
  ...
  $('#player-detail-misc-summary .statistic[name="overallKDA"] .value').text(
    formatStat(
      "KDA",
      playerDetailStats.totalTD / Math.max(playerDetailStats.totalDeaths, 1)
    )
  );
  $('#player-detail-misc-summary .statistic[name="overallMVP"] .value').text(
    formatStat(
      "pct",
      playerDetailStats.totalMVP / Math.max(playerDetailStats.games, 1)
    )
  );
  $('#player-detail-misc-summary .statistic[name="overallAward"] .value').text(
    formatStat(
      "pct",
      playerDetailStats.totalAward / Math.max(playerDetailStats.games)
    )
  );
  ...
  $('#player-detail-misc-summary .statistic[name="votes"] .value').text(
  ...
  $('.statistic[name="soloKills"] .value').text(
    playerDetailStats.takedownHistogram[1]
  );
  $('.statistic[name="soloDeaths"] .value').text(
    playerDetailStats.deathHistogram[1]
  );
...
function renderPlayerHeroDetail() {
  ...
  let mode = $("#player-hero-detail-stats .menu .active.item").attr(
    "data-mode"
  );
  ...
  for (let h in playerDetailStats[mode]) {
    ...
    if (playerDetailStats.heroes[h].games >= playerHeroMatchThreshold)
      $("#player-hero-detail-stats tbody").append(
        playerHeroDetailRowTemplate(context)
      );
  }
...
function renderProgression() {
  ...
  let axis1Stats = $('.player-stat-menu[name="axis1"]')
    .dropdown("get value")
    .split(",");
  let axis2Stats = $('.player-stat-menu[name="axis2"]')
    .dropdown("get value")
    .split(",");
  ...
  DB.getHeroData(query, function(err, docs) {
    ...
    for (let d in docs) {
      ...
      data[hash[0]].mvp +=
        doc.gameStats.awards.indexOf("EndOfMatchAwardMVPBoolean") >= 0 ? 1 : 0;
      data[hash[0]].games += 1;
    ...
    for (let i = 0; i < dataArr.length; i++) {
      ...
      progressionWinRateGraphData.data.datasets[0].data.push(
        (stat.wins / stat.games * 100).toFixed(2)
      );
      progressionWinRateGraphData.data.datasets[1].data.push(stat.games);
      progressionAwardsGraphData.data.datasets[0].data.push(
        (stat.award / stat.games * 100).toFixed(2)
      );
      progressionAwardsGraphData.data.datasets[1].data.push(
        (stat.mvp / stat.games * 100).toFixed(2)
      );

      let idx = 0;
      for (let s of axis1Stats) {
        if (s === "KDA") {
          progressionStatGraphData.data.datasets[idx].data.push(
            (stat.takedowns / Math.max(stat.deaths, 1)).toFixed(2)
          );
        } else {
          progressionStatGraphData.data.datasets[idx].data.push(
            (stat[s] / stat.games).toFixed(2)
          );
        }
  ...
  if (action === "player") {
        ...
        if (files) {
          ...
          DB.getPlayers({}, function(err, docs) {
            for (let i in docs) {
              exportPlayer(
                docs[i]._id,
                path + "/" + docs[i].name + "-" + docs[i]._id + ".json"
              );
            }
          });
        }
      ...
      .modal({
        onApprove: function() {
          dialog.showSaveDialog(
            ...
            function(filename) {
              if (filename) {
                let sections = $("#player-print-sections .ui.dropdown")
                  .dropdown("get value")
                  .split(",");
    ...
    if (playerDetailID) {
      $("#player-set-nickname-modal")
        .find("input")
        .val(playerDetailInfo.nickname);
      $("#player-set-nickname-modal")
        .modal({
          onApprove: function() {
            let name = $("#player-set-nickname-modal")
              .find("input")
              .val();
            ...
            DB.setPlayerNickname(playerDetailID, name, function() {
              ...
              showMessage(
                "Nickname Updated",
                `${playerDetailInfo.name}'s nickname updated to "${name}"`
              );
              updatePlayerPageHeader();
            });
          }
...
function processPlayerCollectionCompare(cache) {
  ...
  if (!cache) {
    showMessage(
      ...
      "Compare to Collection Average panel has no data to compare to."
    );
  }
  ...
  if (activeHero === "all" || activeHero === "") {
    ...
    okdaCtx.pDataSort =
      playerDetailStats.totalTD / Math.max(playerDetailStats.totalDeaths, 1);
    okdaCtx.cmpDataSort =
      cache.heroData.totalTD / Math.max(cache.heroData.totalDeaths, 1);
    lengthCtx.pDataSort =
      playerDetailStats.totalMatchLength / playerDetailStats.games;
    lengthCtx.cmpDataSort =
      cache.heroData.totalMatchLength / cache.heroData.games;
  } else {
    if (!cmpData) {
      ...
      winCtx.cmpDataSort =
        cache.heroData.heroes[activeHero].wins /
        cache.heroData.heroes[activeHero].games;
      okdaCtx.cmpDataSort =
        cache.heroData.total[activeHero].Takedowns /
        Math.max(cache.heroData.total[activeHero].Deaths, 1);
      lengthCtx.cmpDataSort =
        cache.heroData.heroes[activeHero].totalTime /
        cache.heroData.heroes[activeHero].games;
    }

    lengthCtx.pDataSort =
      playerDetailStats.heroes[activeHero].totalTime /
      playerDetailStats.heroes[activeHero].games;
    okdaCtx.pDataSort =
      playerDetailStats.total[activeHero].Takedowns /
      Math.max(playerDetailStats.total[activeHero].Deaths, 1);
    winCtx.pDataSort =
      playerDetailStats.heroes[activeHero].wins /
      playerDetailStats.heroes[activeHero].games;
  }
  ...
  okdaCtx.pctDiff =
    (okdaCtx.pDataSort - okdaCtx.cmpDataSort) / okdaCtx.cmpDataSort;
  ...
  lengthCtx.pctDiff =
    (lengthCtx.pDataSort - lengthCtx.cmpDataSort) / lengthCtx.cmpDataSort;
...
function layoutPlayerPrint(sections) {
  ...
  if ($("#player-detail-summary-header h2").text() === "All Hero Summary") {
    ...
    addPrintHeader(
      "Player Summary: " +
        playerDetailInfo.name +
        "'s " +
        $("#player-hero-select-menu").dropdown("get value")
    );
  }
  addPrintDate();
  ...
  if (sects.indexOf("general") !== -1) {
    ...
    getPrintPage("general").append(
      $("#player-detail-misc-summary .statistics").clone()
    );
    $("#print-window")
      .find(".statistics")
  ...
  if ($("#player-detail-summary-header h2").text() === "All Hero Summary") {
    if (sects.indexOf("pool") !== -1) {
      ...
      getPrintPage("pool").append(
        '<div class="ui horizontal small statistic">' +
          $("#player-overall-hero-pool").html() +
          "</div>"
      );
      getPrintPage("pool")
        .find("#player-overall-hero-pool")
        .removeClass("tiny");
      getPrintPage("pool").append('<div class="ui segment hero-pool"></div>');
      $("#print-window .hero-pool").append(
        $("#player-detail-hero-pool .five.grid").clone()
      );
      $("#print-window .hero-pool").append(
        $("#player-detail-hero-pool .two.grid").clone()
      );

      // graph update / copy
      copyGraph(
        heroPoolGamesGraphData,
        $(".hero-pool #player-hero-pool-games-graph"),
        { width: 500, height: 300 }
      );
      copyGraph(
        heroPoolWinsGraphData,
        $(".hero-pool #player-hero-pool-win-graph"),
        { width: 500, height: 300 }
      );
    }
  } else {
    if (sects.indexOf("talents") !== -1) {
      ...
      copyFloatingTable(
        $("#player-detail-hero-talent .talent-pick .floatThead-wrapper"),
        getPrintPage("talents")
      );
    }
    ...
    if (sects.indexOf("builds") !== -1) {
      ...
      copyFloatingTable(
        $("#player-detail-hero-talent .talent-build .floatThead-wrapper"),
        getPrintPage("builds")
      );
    }
  }
  ...
  if (sects.indexOf("hero") !== -1) {
    ...
    copyFloatingTable(
      $("#player-detail-hero-summary .floatThead-wrapper"),
      getPrintPage("hero")
    );
  }
  ...
  if (sects.indexOf("maps") !== -1) {
    ...
    copyFloatingTable(
      $("#player-detail-map-summary .floatThead-wrapper"),
      getPrintPage("maps")
    );
  }
  ...
  if (sects.indexOf("with") !== -1) {
    ...
    copyFloatingTable(
      $("#player-detail-with-summary .floatThead-wrapper"),
      getPrintPage("with")
    );
  }
  ...
  if (sects.indexOf("against") !== -1) {
    ...
    copyFloatingTable(
      $("#player-detail-against-summary .floatThead-wrapper"),
      getPrintPage("against")
    );
  }
  ...
  if (sects.indexOf("awards") !== -1) {
    ...
    copyFloatingTable(
      $("#player-detail-award-summary .floatThead-wrapper"),
      getPrintPage("award")
    );
  }
  ...
  if (sects.indexOf("compare") !== -1) {
    ...
    addPrintSubHeader(
      "Comparison vs Collection Average: " +
        $("#player-compare-collection").dropdown("get text"),
      "compare"
    );
    copyFloatingTable(
      $("#player-compare-segment .floatThead-wrapper"),
      getPrintPage("compare")
    );
  }
  ...
  if (sects.indexOf("with-player") !== -1) {
    ...
    copyFloatingTable(
      $("#player-detail-friend-summary .floatThead-wrapper"),
      getPrintPage("with-player")
    );
  }
  ...
  if (sects.indexOf("against-player") !== -1) {
    ...
    copyFloatingTable(
      $("#player-detail-rival-summary .floatThead-wrapper"),
      getPrintPage("against-player")
    );
  }
  ...
  if (sects.indexOf("skins") !== -1) {
    ...
    copyFloatingTable(
      $("#player-detail-skin-summary .floatThead-wrapper"),
      getPrintPage("skins")
    );
  }
  ...
  if (sects.indexOf("taunts") !== -1) {
    ...
    getPrintPage("taunts").append(
      '<div class="ui segment detail-taunt"></div>'
    );
    $("#print-window .contents .detail-taunt").append(
      $("#player-detail-taunt-summary").clone()
    );
