...
function loadDatabase() {
  ...
  Heroes = new HeroesTalents.HeroesTalents(
...
function loadDatabaseComplete(err) {
  if (err) {
    showMessage(
      ...
      'Try restarting the app, and if the issue persists, please file a bug report with this message: "' +
        err +
        '"',
...
function loadSections() {
  ...
  $("#main-content").append(
    getTemplate("hero-collection", "#hero-collection-page")
  );
  initHeroCollectionPage();

  $("#main-content").append(
    getTemplate("player-ranking", "#player-ranking-page")
  );
  ...
  sections.maps = {
    id: "#maps-page-content",
    title: "Battlegrounds",
    showBack: false,
    onShow: onShowMapsPage
  };
...
function globalDBUpdate() {
  // patch update
  addPatchMenuOptions(
    $("#filter-popup-widget .filter-widget-patch"),
    function() {
...
function updatePlayerMenuOptions(elem, value) {
  ...
  DB.getPlayers(
    { $or: [{ name: { $regex: q } }, { nickname: { $regex: q } }] },
...
function addMapMenuOptions(menu) {
  ...
  for (let m in ReplayTypes.MapType) {
    ...
    menu
      .find(".menu")
      .append('<div class="item" data-value="' + map + '">' + map + "</div>");
  }
}
  ...
  DB.getVersions(function(versions) {
    ...
    for (let v in versions) {
      elem
        .find(".menu")
        .append(
          '<div class="item" data-value="' + v + '">' + versions[v] + "</div>"
        );
    }
...
function updateCollectionMenu(callback) {
  ...
  DB.getCollections(function(err, collections) {
    ...
    $("#collection-switch-menu .menu").append(
      '<div class="item" data-value="none">All Matches</div>'
    );
    ...
    $("#settings-collection-import .menu").prepend(
      '<div class="item" data-value="">[No Collection]</div>'
    );
...
function populateTagMenu(menu, callback) {
  DB.getTags(function(tags) {
    ...
    for (let tag of tags) {
      menu
        .find(".menu")
        .append('<div class="item" data-value="' + tag + '">' + tag + "</div>");
    }
...
function exportMatch(id, filename) {
  DB.getMatchesByID([id], function(err, docs) {
    ...
    DB.getHeroDataForID(id, function(err, docs) {
      ...
      fs.writeFile(filename, JSON.stringify(matchExport, null, 2), function(
        err
      ) {
        if (err) {
...
function addPrintPage(name) {
  $("#print-window .contents").append(
    '<div class="new-page page ' + name + '"></div>'
  );
}

function addPrintDate() {
  $("#print-window .contents").append(
    "<p>Database path: " + settings.get("dbPath") + "</p>"
  );
  $("#print-window .contents").append(
...
function renderAndPrint(filename, size = "Letter", landscape = false) {
  ...
  win.webContents.printToPDF({ landscape: landscape, pageSize: size }, function(
    error,
    data
  ) {
    if (error) {
      ...
      try {
        ...
        showMessage("Print Success", "Printed to " + filename, {
          class: "positive"
        });
      } catch (err) {
