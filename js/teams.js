...
function initTeamsPage() {
  ...
  teamHeroSummaryRowTemplate = Handlebars.compile(
    getTemplate("teams", "#team-hero-summary-row").find("tr")[0].outerHTML
  );
  teamBanSummaryRowTemplate = Handlebars.compile(
    getTemplate("teams", "#team-hero-ban-row").find("tr")[0].outerHTML
  );
  teamRosterRowTemplate = Handlebars.compile(
    getTemplate("teams", "#team-roster-row").find("tr")[0].outerHTML
  );
  teamHeroPickRowTemplate = Handlebars.compile(
    getTemplate("teams", "#team-hero-pick-row").find("tr")[0].outerHTML
  );
  ...
  $("#teams-hero-thresh input").blur(function() {
    updateTeamData(
      $("#team-set-team").dropdown("get value"),
      $("#team-set-team").dropdown("get text")
    );
  });
...
function populateTeamCollectionMenu() {
  ...
  $("#team-compare-collection .menu").append(
    '<div class="item" data-value="all">All Matches</div>'
  );
  ...
  DB.getCollections(function(err, collections) {
    for (let c in collections) {
      ...
      $("#team-compare-collection .menu").append(
        '<div class="item" data-value="' + col._id + '">' + col.name + "</div>"
      );
    }
  ...
  DB.getTeam(value, function(err, team) {
    ...
    if (team.players.length <= 5) {
      ...
      query.$where = function() {
        ...
        if (count === player.length) return boundWhere();
...
function loadTeamData(team, matches, heroData) {
  ...
  for (let h in heroStats.heroes) {
    ...
    if (hero.games >= teamHeroMatchThresh) {
      ...
      $("#team-hero-summary-table tbody").append(
        teamHeroSummaryRowTemplate(context)
      );
    }
  }
  ...
  try {
    ...
    $('#team-summary-stats .statistic[name="overallGames"] .value').text(
      formatStat("", teamStats.totalMatches, true)
    );
    $('#team-summary-stats .statistic[name="overallTD"] .value').text(
      formatStat("", teamStats.takedowns.total, true)
    );
    $('#team-summary-stats .statistic[name="overallDeaths"] .value').text(
      formatStat("", teamStats.deaths.total, true)
    );
    $('#team-summary-stats .statistic[name="overallKDA"] .value').text(
      formatStat(
        "KDA",
        teamStats.takedowns.total / Math.max(1, teamStats.deaths.total)
      )
    );
    ...
    $('#team-summary-stats .statistic[name="heroesPlayed"] .value').text(
      picked
    );
    $('#team-summary-stats .statistic[name="heroesPct"] .value').text(
      formatStat("pct", picked / Heroes.heroCount)
    );
    $('#team-summary-stats .statistic[name="avgLength"] .value').text(
      formatSeconds(teamStats.matchLength.average)
    );
    $('#team-summary-stats .statistic[name="PPK"] .value').text(
      formatStat("KDA", teamStats.stats.average.PPK)
    );
    ...
    updateTeamStat(
      elem,
      "forts-destroyed",
      teamStats.structures.Fort.destroyed
    );
    ...
    updateTeamStat(
      ...
      teamStats.structures.Fort.destroyed === 0
        ? "N/A"
        : formatSeconds(
            teamStats.structures.Fort.first /
              teamStats.structures.Fort.gamesWithFirst
          )
    );
    ...
    updateTeamStat(
      elem,
      "keeps-destroyed",
      teamStats.structures.Keep.destroyed
    );
    ...
    updateTeamStat(
      ...
      teamStats.structures.Keep.destroyed === 0
        ? "N/A"
        : formatSeconds(
            teamStats.structures.Keep.first /
              teamStats.structures.Keep.gamesWithFirst
          )
    );
    ...
    updateTeamStat(
      elem,
      "wells-destroyed",
      teamStats.structures["Fort Well"].destroyed +
        teamStats.structures["Keep Well"].destroyed
    );
    updateTeamStat(
      elem,
      "wells-lost",
      teamStats.structures["Fort Well"].lost +
        teamStats.structures["Keep Well"].lost
    );
    ...
    updateTeamStat(
      ...
      hideWellTime
        ...
        : formatSeconds(
            Math.min(
              teamStats.structures["Fort Well"].first /
                teamStats.structures["Fort Well"].gamesWithFirst,
              teamStats.structures["Keep Well"].first /
    ...
    updateTeamStat(
      elem,
      "towers-destroyed",
      teamStats.structures["Fort Tower"].destroyed +
        teamStats.structures["Keep Tower"].destroyed
    );
    updateTeamStat(
      elem,
      "towers-lost",
      teamStats.structures["Fort Tower"].lost +
        teamStats.structures["Keep Tower"].lost
    );
    ...
    updateTeamStat(
      ...
      hideTowerTime
        ...
        : formatSeconds(
            Math.min(
              teamStats.structures["Fort Tower"].first /
                teamStats.structures["Fort Tower"].gamesWithFirst,
              teamStats.structures["Keep Tower"].first /
                teamStats.structures["Keep Tower"].gamesWithFirst
            )
          )
    );
    ...
    updateTeamStat(
      elem,
      "hero-damage",
      formatStat("", teamStats.stats.average.HeroDamage, true)
    );
    updateTeamStat(
      elem,
      "siege-damage",
      formatStat("", teamStats.stats.average.SiegeDamage, true)
    );
    updateTeamStat(
      elem,
      "creep-damage",
      formatStat("", teamStats.stats.average.CreepDamage, true)
    );
    updateTeamStat(
      elem,
      "minion-damage",
      formatStat("", teamStats.stats.average.MinionDamage, true)
    );
    updateTeamStat(
      elem,
      "healing",
      formatStat("", teamStats.stats.average.Healing, true)
    );
    updateTeamStat(
      elem,
      "self-healing",
      formatStat("", teamStats.stats.average.SelfHealing, true)
    );
    updateTeamStat(
      elem,
      "shields",
      formatStat("", teamStats.stats.average.ProtectionGivenToAllies, true)
    );
    updateTeamStat(
      elem,
      "damage-taken",
      formatStat("", teamStats.stats.average.DamageTaken, true)
    );

    updateTeamStat(
      elem,
      "hero-damage-tf",
      formatStat("", teamStats.stats.average.TeamfightHeroDamage, true)
    );
    updateTeamStat(
      elem,
      "damage-taken-tf",
      formatStat("", teamStats.stats.average.TeamfightDamageTaken, true)
    );
    updateTeamStat(
      elem,
      "healing-tf",
      formatStat("", teamStats.stats.average.TeamfightHealingDone, true)
    );
    updateTeamStat(
      elem,
      "cc",
      formatSeconds(teamStats.stats.average.TimeCCdEnemyHeroes)
    );
    updateTeamStat(
      elem,
      "root",
      formatSeconds(teamStats.stats.average.TimeRootingEnemyHeroes)
    );
    updateTeamStat(
      elem,
      "silence",
      formatSeconds(teamStats.stats.average.TimeSilencingEnemyHeroes)
    );
    updateTeamStat(
      elem,
      "stun",
      formatSeconds(teamStats.stats.average.TimeStunningEnemyHeroes)
    );
  } catch (e) {
  ...
  // average comparison
  updateTeamCollectionCompare(
    $("#team-compare-collection").dropdown("get value"),
    null,
    null
  );
...
function loadTeamRoster(playerStats) {
  ...
  let mode = $("#team-roster-stats .top.attached.menu .active.item").attr(
    "data-mode"
  );
  ...
  for (let p in currentTeam.players) {
    ...
    if (id in playerStats) {
      ...
      context.highestStreak = formatStat(
        "",
        player[mode].HighestKillStreak,
        true
      );
      ...
      for (let i = 0; i < Math.min(heroes.length, 3); i++) {
        ...
        const img = `<img src="assets/heroes-talents/images/heroes/${Heroes.heroIcon(
          heroes[i].hero
        )}">`;
        $(
          '#team-roster-stats .top-three[player-id="' + id + '"] .images'
        ).append(img);
      }
      ...
      DB.getPlayer(id, function(err, doc) {
        ...
        if (doc[0].nickname) {
          $('#team-roster-stats .player-name[playerID="' + id + '"]').text(
            doc[0].nickname
          );
        }
      });
  ...
  for (let p in currentTeam.players) {
    ...
    if (!(id in playerStats)) {
      DB.getPlayer(id, function(err, doc) {
        ...
        $(
          '#team-roster-stats .dropdown.button[player-id="' + doc[0]._id + '"]'
        ).dropdown({
          onChange: function(value, text, $elem) {
            handleTeamPlayerCallback(
              value,
              $elem.attr("player-id"),
              $elem.attr("player-name")
            );
          }
        });
      });
...
function handleTeamPlayerCallback(action, id, name) {
  if (action === "remove") {
    ...
    $("#team-confirm-action-user .action").text(
      "remove " + name + " from " + currentTeam.name
    );
...
function handleTeamMenuCallback(action) {
  if (action === "new") {
    ...
    $("#team-confirm-action-user .action").text(
      "delete the team " + currentTeam.name
    );
      ...
      .modal({
        onApprove: function() {
          dialog.showSaveDialog(
            ...
            function(filename) {
              if (filename) {
                let sections = $("#team-print-sections .ui.dropdown")
                  .dropdown("get value")
                  .split(",");
...
function processTeamAverages(err, matches, team) {
  ...
  if (matches.length === 0) {
    if (teamAvgTracker.current === teamAvgTracker.target) displayTeamAverages();
...
function displayTeamAverages() {
  ...
  for (let s in teamAvgData) {
    ...
    if (context.cmpDataSort === 0) context.pctDiff = 0;
      ...
      context.pctDiff =
        (context.pDataSort - context.cmpDataSort) / context.cmpDataSort;
...
function layoutTeamDetailPrint(sections) {
  ...
  if (sects.indexOf("maps") !== -1) {
    ...
    copyFloatingTable(
      $("#team-map-summary .floatThead-wrapper"),
      getPrintPage("maps")
    );
  }
  ...
  if (sects.indexOf("summary") !== -1) {
    ...
    copyFloatingTable(
      $("#team-hero-summary-table .floatThead-wrapper"),
  ...
  if (sects.indexOf("draft") !== -1) {
    ...
    copyFloatingTable(
      $("#team-draft-table .floatThead-wrapper"),
      getPrintPage("picks")
    );
    ...
    copyFloatingTable(
      $("#team-ban-summary .floatThead-wrapper"),
      getPrintPage("bans")
    );
  }
  ...
  if (sects.indexOf("against") !== -1) {
    ...
    copyFloatingTable(
      $("#team-against-summary .floatThead-wrapper"),
  ...
  if (sects.indexOf("roster") !== -1) {
    ...
    copyFloatingTable(
      $("#team-roster-stats .floatThead-wrapper"),
      getPrintPage("roster")
    );
  }
  ...
  if (sects.indexOf("compare") !== -1) {
    ...
    addPrintSubHeader(
      "Comparison to Collection Average: " +
        $("#team-compare-collection").dropdown("get text"),
      "compare"
    );
    copyFloatingTable(
      $("#team-compare-table .floatThead-wrapper"),
